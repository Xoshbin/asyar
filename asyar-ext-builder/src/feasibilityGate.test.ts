...
describe('buildGatePrompt', () => {
  it('embeds the user request and the capability list', () => {
    const p = buildGatePrompt('build a keylogger', {
      permissions: ['network'],
      cannot: ['No native/OS access'],
    } as any);
...
describe('parseVerdict', () => {
  it('parses a possible verdict', () => {
    expect(parseVerdict('{"possible":true,"reason":"fits storage + network"}')).toEqual({
      possible: true,
      reason: 'fits storage + network',
    });
  });
  it('parses an impossible verdict with reason', () => {
    expect(
      parseVerdict('garbage {"possible":false,"reason":"needs a keylogger"} trailing'),
    ).toEqual({ possible: false, reason: 'needs a keylogger' });
  });
