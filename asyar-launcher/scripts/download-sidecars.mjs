...
async function ensureSidecar(platform, { repo, archive, binaryName }) {
  ...
  if (existsSync(destPath)) {
    console.log(
      `  ${binaryName} (${platform.platformKey}): already exists at binaries/${destName}, skipping`,
    );
    return;
  }
...
function lipoUniversal({ universalTriple, sourceTriples }, binaryName) {
  ...
  if (existsSync(destPath)) {
    console.log(
      `  ${binaryName} (${universalTriple}): already exists at binaries/${destName}, skipping`,
    );
    return;
  }
  ...
  console.log(
    `  ${binaryName} (${universalTriple}): lipo -create from ${sourceTriples.join(' + ')}...`,
  );
...
async function resolveClaudeVersion() {
  ...
  if (!/^\d+\.\d+\.\d+$/.test(version)) {
    throw new Error(
      `Unexpected claude version "${version}" (from ${pinned ? 'CLAUDE_CODE_VERSION' : `${CLAUDE_BASE_URL}/latest`})`,
    );
  }
  return version;
}
...
async function fetchClaudeManifest(version) {
  const json = await downloadText(
    `${CLAUDE_BASE_URL}/${version}/manifest.json`,
    `manifest-${version}`,
  );
...
async function ensureClaude(platform, version, manifest) {
  ...
  if (existsSync(destPath)) {
    console.log(
      `  claude (${platform.platformKey}): already exists at binaries/${destName}, skipping`,
    );
    return;
  }
