...
console.log(
  `Launcher release: ${pkg.version} → ${version} (tag ${tag}) · local SDK ${sdkVersion}${dryRun ? '  [dry-run]' : ''}`,
);
...
try {
  ...
  if (dryRun) {
    ...
    console.log(
      '[dry-run] would update scaffoldService.ts offline fallback, Cargo.toml, Cargo.lock',
    );
  } else {
    ...
    if (existsSync(scaffoldPath)) {
      ...
      const after = before.replace(
        ...
        `return '^${sdkVersion}'; // Offline fallback`,
      );
      if (after !== before) {
