...
describe('fromKeyboardEvent', () => {
  ...
  it('builds Control+C from ctrlKey + c', () => {
    expect(fromKeyboardEvent(makeEvent({ key: 'c', code: 'KeyC', ctrlKey: true }))).toBe(
      'Control+C',
    );
  });

  it('builds Alt+Space from altKey + space', () => {
    expect(fromKeyboardEvent(makeEvent({ key: ' ', code: 'Space', altKey: true }))).toBe(
      'Alt+Space',
    );
  });

  it('builds Shift+A from shiftKey + a', () => {
    expect(fromKeyboardEvent(makeEvent({ key: 'A', code: 'KeyA', shiftKey: true }))).toBe(
      'Shift+A',
    );
  });
  ...
  it('combines multiple modifiers in correct order (Super > Control > Alt > Shift)', () => {
    const result = fromKeyboardEvent(
      makeEvent({ key: 'k', code: 'KeyK', metaKey: true, shiftKey: true }),
    );
