...
export async function setUpHiddenCommand(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ok: false,
        error: `Couldn't bind ${modifier}+${key}.${who} Try a different hotkey.`,
      };
    }
    return { ok: true };
  } catch (e) {
