  ...
  <div class="mb-8 p-6">
    ...
    <div class="flex justify-center gap-6">
      <label class="flex flex-col items-center cursor-pointer">
        <div
          class="w-36 h-32 rounded-xl border-2 {handler.selectedTheme === 'system'
            ? 'border-[var(--accent-primary)]'
            : 'border-transparent hover:border-[var(--border-color)]'} bg-gradient-to-r from-[#f8f9fa] to-[#212529] mb-2 flex items-center justify-center shadow-sm overflow-hidden"
        >
          <div
            class="w-full h-full flex items-center justify-center"
            style="background: linear-gradient(to right, #f8f9fa, #212529);"
          >
      ...
      <label class="flex flex-col items-center cursor-pointer">
        <div
          class="w-36 h-32 rounded-xl border-2 {handler.selectedTheme === 'light'
            ? 'border-[var(--accent-primary)]'
            : 'border-transparent hover:border-[var(--border-color)]'} mb-2 flex items-center justify-center shadow-sm overflow-hidden"
      ...
      <label class="flex flex-col items-center cursor-pointer">
        <div
          class="w-36 h-32 rounded-xl border-2 {handler.selectedTheme === 'dark'
            ? 'border-[var(--accent-primary)]'
            : 'border-transparent hover:border-[var(--border-color)]'} mb-2 flex items-center justify-center shadow-sm overflow-hidden"
...
<SettingsSection title="Launch View">
  <div class="mb-8 p-6">
    <div class="flex justify-center gap-6">
      <label class="flex flex-col items-center cursor-pointer">
        <div
          class="w-40 h-32 rounded-xl border-2 p-4 flex items-start justify-center {handler.selectedLaunchView ===
          'default'
            ? 'border-[var(--accent-primary)]'
        ...
        >
          <div
            class="w-24 rounded-lg overflow-hidden"
            style="background: var(--bg-secondary); box-shadow: var(--shadow-sm); border: 1px solid var(--border-color);"
          >
            <div
              class="h-5 flex items-center px-2.5"
            ...
            <div
              class="h-4 flex items-center justify-end gap-1 px-2"
        ...
        <input
          ...
          name="launchView"
          value="default"
          checked={handler.selectedLaunchView === 'default'}
          onchange={() => selectLaunchView('default')}
          class="sr-only"
        />
      ...
      <label class="flex flex-col items-center cursor-pointer">
        ...
        >
          ...
          >
            ...
            <div
              class="h-4 flex items-center justify-end px-2"
              style="border-top: 1px solid var(--border-color);"
            >
              <div class="h-1 w-1 rounded-full" style="background: var(--border-color);"></div>
            </div>
        ...
        <input
          ...
          name="launchView"
          value="compact"
          checked={handler.selectedLaunchView === 'compact'}
          onchange={() => selectLaunchView('compact')}
          class="sr-only"
        />
  ...
  <SettingsSection title="Custom Themes">
    <div class="p-6">
      <div class="grid gap-3">
        <label
          class="flex items-center p-3 rounded-lg cursor-pointer border-2 {activeThemeId === null
            ? 'border-[var(--accent-primary)]'
            : 'border-transparent hover:border-[var(--border-color)]'}"
        ...
        {#each themeExtensions as theme}
          <label
            class="flex items-center p-3 rounded-lg cursor-pointer border-2 {activeThemeId ===
            theme.id
              ? 'border-[var(--accent-primary)]'
              : 'border-transparent hover:border-[var(--border-color)]'}"
            style="background: var(--bg-secondary);"
