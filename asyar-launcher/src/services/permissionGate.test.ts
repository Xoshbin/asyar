...
describe('checkPermission', () => {
  ...
  describe('clipboard:read', () => {
    it('allows readCurrentClipboard when clipboard:read is declared', () => {
      expect(
        checkPermission('ext', 'asyar:api:clipboard:readCurrentClipboard', ['clipboard:read'])
          .allowed,
      ).toBe(true);
    });
    ...
    it('allows getRecentItems when clipboard:read is declared', () => {
      expect(
        checkPermission('ext', 'asyar:api:clipboard:getRecentItems', ['clipboard:read']).allowed,
      ).toBe(true);
    });

    it('allows readCurrentText when clipboard:read is declared', () => {
      expect(
        checkPermission('ext', 'asyar:api:clipboard:readCurrentText', ['clipboard:read']).allowed,
      ).toBe(true);
    });
  ...
  describe('shell:spawn', () => {
    ...
    it('allows raw Tauri invoke when shell:spawn is declared', () => {
      expect(checkPermission('trusted-ext', 'asyar:api:invoke', ['shell:spawn']).allowed).toBe(
        true,
      );
    });
  ...
  describe('denial reason message', () => {
    it('includes the extension ID', () => {
      const result = checkPermission(
        'pomodoro-timer',
        'asyar:api:clipboard:readCurrentClipboard',
        [],
      );
  ...
  describe('application (canonical namespace)', () => {
    it('blocks getFrontmostApplication without application:read', () => {
      const result = checkPermission(
        'test-ext',
        'asyar:api:application:getFrontmostApplication',
        [],
      );
      expect(result.allowed).toBe(false);
  ...
  describe('entitlements:read', () => {
    it('allows entitlements:check when entitlements:read is declared', () => {
      expect(
        checkPermission('ext', 'asyar:api:entitlements:check', ['entitlements:read']).allowed,
      ).toBe(true);
    });
  ...
  describe('oauth:use', () => {
    ...
    it('allows oauth:revokeToken when oauth:use is declared', () => {
      expect(checkPermission('ext', 'asyar:api:oauth:revokeToken', ['oauth:use']).allowed).toBe(
        true,
      );
    });
  });
  ...
  describe('extension:invoke', () => {
    ...
    it('allows interop:launchCommand when extension:invoke is declared', () => {
      expect(
        checkPermission('ext', 'asyar:api:interop:launchCommand', ['extension:invoke']).allowed,
      ).toBe(true);
    });
  });
  ...
  describe('selection:read', () => {
    it('allows selection:getSelectedText with selection:read', () => {
      expect(
        checkPermission('ext', 'asyar:api:selection:getSelectedText', ['selection:read']).allowed,
      ).toBe(true);
    });
  ...
  describe('preferences:read/write', () => {
    it('allows preferences:getAll with preferences:read', () => {
      expect(
        checkPermission('ext', 'asyar:api:preferences:getAll', ['preferences:read']).allowed,
      ).toBe(true);
    });
    ...
    it('allows preferences:set with preferences:write', () => {
      expect(
        checkPermission('ext', 'asyar:api:preferences:set', ['preferences:write']).allowed,
      ).toBe(true);
    });
  ...
  describe('power:inhibit', () => {
    ...
    it('allows keepAwake with power:inhibit', () => {
      expect(checkPermission('ext', 'asyar:api:power:keepAwake', ['power:inhibit']).allowed).toBe(
        true,
      );
    });
  });
  ...
  describe('browser permission gating', () => {
    ...
    it('browser:listBookmarks requires browser:bookmarks.read', () => {
      ...
      const allowed = checkPermission('ext.id', 'asyar:api:browser:listBookmarks', [
  ...
  describe('browser subscribe wire names — per-kind permission', () => {
    it('browser:subscribeTabsChanged requires browser:tabs.read', () => {
      expect(checkPermission('e', 'asyar:api:browser:subscribeTabsChanged', []).allowed).toBe(
        false,
      );
      expect(
        checkPermission('e', 'asyar:api:browser:subscribeTabsChanged', ['browser:tabs.read'])
          .allowed,
      ).toBe(true);
    });

    it('browser:unsubscribeTabsChanged requires browser:tabs.read', () => {
      expect(checkPermission('e', 'asyar:api:browser:unsubscribeTabsChanged', []).allowed).toBe(
        false,
      );
    });

    it('browser:subscribePageChanged requires browser:page.read', () => {
      expect(checkPermission('e', 'asyar:api:browser:subscribePageChanged', []).allowed).toBe(
        false,
      );
      expect(
        checkPermission('e', 'asyar:api:browser:subscribePageChanged', ['browser:page.read'])
          .allowed,
      ).toBe(true);
    });

    it('browser:unsubscribePageChanged requires browser:page.read', () => {
      expect(checkPermission('e', 'asyar:api:browser:unsubscribePageChanged', []).allowed).toBe(
        false,
      );
    });
    ...
    it('browser:tabs.read does NOT grant subscribe to page events (anti-side-channel)', () => {
      expect(
        checkPermission('e', 'asyar:api:browser:subscribePageChanged', ['browser:tabs.read'])
          .allowed,
      ).toBe(false);
    });
  ...
  describe('browser command-bar permission gating', () => {
    it('browser:searchWeb requires browser:tabs.write', () => {
      ...
      expect(
        checkPermission('e', 'asyar:api:browser:searchWeb', ['browser:tabs.write']).allowed,
      ).toBe(true);
    });
    it('browser:getMostRecentActiveBrowser requires browser:tabs.read', () => {
      expect(checkPermission('e', 'asyar:api:browser:getMostRecentActiveBrowser', []).allowed).toBe(
        false,
      );
      expect(
        checkPermission('e', 'asyar:api:browser:getMostRecentActiveBrowser', ['browser:tabs.read'])
          .allowed,
      ).toBe(true);
    });
  });
  ...
  describe('snippets:contribute', () => {
    ...
    it('allows registerShortcodes when permission declared', () => {
      const result = checkPermission('org.asyar.emoji', 'asyar:api:snippets:registerShortcodes', [
        'snippets:contribute',
      ]);
      expect(result.allowed).toBe(true);
    });
