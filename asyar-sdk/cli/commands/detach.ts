  ...
  program
    ...
    .action((idOrPath: string | undefined, opts: any) => {
      try {
        ...
        let registry: Record<string, string> = JSON.parse(
          fs.readFileSync(devExtensionsFile, 'utf-8'),
        );
        ...
        if (!identifier) {
          try {
            ...
            console.log(
              chalk.red(
                'Provide an extension ID, path, or run from inside an extension directory.',
              ),
            );
            return;
          }
        ...
        if (!removedId) {
          console.log(
            chalk.yellow(`Extension "${identifier}" is not registered as a dev extension.`),
          );
          return;
        }
