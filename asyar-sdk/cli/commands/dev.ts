  ...
  program
    ...
    .action(async () => {
      ...
      if (errors.length > 0) {
        ...
        errors.forEach((e) => console.log(chalk.yellow(`  ⚠ ${e.field}: ${e.message}`)));
      }

      const isAlreadyLinked = fs.existsSync(targetDir) && fs.lstatSync(targetDir).isSymbolicLink();
      ...
      if (manifest.type === 'theme') {
        ...
        console.log(
          chalk.gray('Tip: re-select your theme in Settings → Appearance to preview updates\n'),
        );

        const watcher = chokidar.watch(
          [path.join(cwd, 'theme.json'), path.join(cwd, 'manifest.json')],
          { ignoreInitial: true },
        );
