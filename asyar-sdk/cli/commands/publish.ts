...
import { getExtensionRepoUrl, saveExtensionRepoUrl } from '../lib/config';
import {
  isOwnGitRoot,
...
export function registerPublish(program: Command) {
  program
    ...
    .action(async (opts) => {
        ...
        if (remoteUrl && /github\.com\//.test(remoteUrl)) {
          ...
          if (storedRepoUrl) {
            ...
            if (parentOrigin && storedRepoUrl === parentOrigin) {
              console.log(
                chalk.yellow('⚠') +
                  ` Stored repo equals the parent monorepo's origin (${parentOrigin}) — discarding stale entry and auto-creating a dedicated repo for this extension.`,
              );
              storedRepoUrl = null;
            }
          }
          ...
          if (storedRepoUrl) {
            ...
            const repoName = 'asyar-' + manifest.id.split('.').pop() + '-extension';
      ...
      pkgSpinner.succeed(`Extension packaged (${(sizeBytes / 1024).toFixed(1)} kB)`);
      ...
      if (existingRelease) {
        ...
        const existingAsset = existingRelease.assets?.find((a: any) => a.name.endsWith('.zip'));
