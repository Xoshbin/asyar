...
export async function login(): Promise<{
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(CLI_PORT, () => {
      const authUrl =
        `${STORE_URL}/auth/github` + `?redirect=http://localhost:${CLI_PORT}/callback`;
...
export async function getOrAuthorizeGitHub(): Promise<string> {
  ...
  console.log();
  console.log(chalk.bgBlue.white.bold(` ${user_code} `));
  console.log();
  ...
  while (Date.now() < expiresAt) {
    ...
    try {
      pollResponse = await fetch('https://github.com/login/oauth/access_token', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/x-www-form-urlencoded',
      ...
      throw new Error(
        'Could not connect to GitHub during authorization. Please check your internet connection.',
      );
    }
    ...
    if (result.access_token) {
      ...
      const keytar = require('keytar');
      await keytar.setPassword(KEYCHAIN_SERVICE, KEY_GITHUB_TOKEN, result.access_token);
