...
export class ActionServiceProxy extends BaseServiceProxy implements IActionService {
  ...
  registerAction(action: ExtensionAction): void {
    ...
    this.broker
      .invoke('actions:registerAction', { action: actionData })
      .catch((err) => console.warn('[ActionServiceProxy] registerAction failed:', err));
  }

  unregisterAction(actionId: string): void {
    extensionBridge.unregisterAction(actionId);
    this.broker
      .invoke('actions:unregisterAction', { actionId })
      .catch((err) => console.warn('[ActionServiceProxy] unregisterAction failed:', err));
  }
  ...
  setContext(context: ActionContext, data?: { commandId?: string }): void {
    ...
    this.broker
      .invoke('actions:setContext', { context, data })
      .catch((err) => console.warn('[ActionServiceProxy] setContext failed:', err));
  }
  ...
  registerActionHandler(
    ...
    this.broker
      .invoke('actions:registerActionHandler', { actionId })
      .catch((err) =>
