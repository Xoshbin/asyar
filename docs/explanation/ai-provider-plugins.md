...
The same boundary translates the shared `ProviderConfig.reasoningEffort` setting: OpenAI Responses and OpenRouter receive `reasoning.effort`, OpenAI-compatible Chat Completions receives `reasoning_effort`, Anthropic receives `output_config.effort`, Gemini receives `generationConfig.thinkingConfig.thinkingLevel`, and Ollama receives `think`. A missing value must be omitted rather than replaced with a guessed default.
