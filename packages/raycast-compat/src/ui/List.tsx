...
export function ListEmptyView(props: ListEmptyViewProps): React.ReactElement {
  return (
    ...
    >
      {props.icon && <div style={{ fontSize: '32px' }}>{String(props.icon)}</div>}
      {props.title && (
                ...
                <React.Fragment key={item.id}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      {item.icon && <span style={{ fontSize: '16px' }}>{String(item.icon)}</span>}
                      <span
                        style={{
                          fontSize: '13px',
                    ...
                    {item.accessories && item.accessories.length > 0 && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {item.accessories.map((acc, aIdx) => (
                          ...
                          >
                            {acc.icon && (
                              <span style={{ marginRight: '4px' }}>{String(acc.icon)}</span>
                            )}
                            {acc.text}
                          </span>
                        ))}
